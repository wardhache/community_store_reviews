...
class Review
{
    ...
    public function getCurrentStatus() {
      ...
      $status = $db->fetchAssoc("SELECT rshID, rID, rsID FROM CommunityStoreReviewStatusHistories rsh WHERE rsh.rID = ? ORDER BY rshID DESC LIMIT 0,1;", array($this->getID()));
    ...
    public function saveReview($data)
    {
        ...
        if($data['pID']) {
            ...
            if($data['rsID']) {
              ...
              if($autoApprove == 1 && !empty($autoApproveRating) && trim($autoApproveRating) != '' && $autoApproveRating <= $averageRating) {
            ...
            $reviewReminder = Config::get('community_store_review.reminder');
